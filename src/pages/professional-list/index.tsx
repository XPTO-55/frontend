...
export default function ProfessionaiList() {
  ...
  const filteredProfessionalList =
    search.length > 0
      ? mockListProfessinals.filter((accordion) =>
          accordion.nome.includes(search)
        )
      : mockListProfessinals;
    ...
    <>
      ...
      <div>
        <S.PageContainer>
          ...
          <S.Container>
            {filteredProfessionalList.map((res, index) => {
              return (
                <ProfessionalCard key={index} nome={res.nome} image={res.img} />
              );
            })}
          </S.Container>
        </S.PageContainer>
